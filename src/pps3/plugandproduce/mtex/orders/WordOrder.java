...
public class WordOrder {
    private final int OVERPRINT_MIN = 1;
    private final int OVERPRINT_MAX = 9;
    private final int STEP_COUNT_MIN = 1;
    private final int STEP_COUNT_MAX = 8;
    private final int OVEN_TEMPERATURE_MIN = 1;
    private final int OVEN_TEMPERATURE_MAX = 9999;
    ...
    public WordOrder(String orderId, int quantity, String ripId, int overprint, int stepCount, int ovenTemperature) throws OverprintOutsideRange, StepCountOutsideRange, OvenTemperatureOutsideRange {
        if (overprint > OVERPRINT_MAX || overprint < OVERPRINT_MIN) {
            throw new OverprintOutsideRange("The overprint value ("+overprint+") is outside the defined boundaries ["+OVERPRINT_MIN+","+OVERPRINT_MAX+"].");
        }

        if (stepCount > STEP_COUNT_MAX || stepCount < STEP_COUNT_MIN) {
            throw new StepCountOutsideRange("The stepCount value ("+stepCount+") is outside the defined boundaries ["+STEP_COUNT_MIN+","+STEP_COUNT_MAX+"].");
        }
    ...
    public int calculateChecksum16Bits() {
        ...
        for (int i = 0; i < this.orderId.length(); i++) {
            checksum += (int) (this.orderId.charAt(i));
        }

        for (int i = 0; i < this.ripId.length(); i++) {
            checksum += (int) (this.ripId.charAt(i));
        }

        checksum += this.quantity + this.overprint + this.stepCount + this.ovenTemperature;

        int highPart = -1, shortPart;

        while (highPart != 0) {
            highPart = checksum >> 16;
            shortPart = checksum & (0xFFFF);

            checksum = shortPart + highPart;
        }

        return (0xFFFF) & ~checksum;
    }

    public int calculateCrcModbus16Bits() {
        int[] table = {
        ...
        int crc = 0xFFFF;
        final int SIZE_BYTES_INT = 4;
        final int SIZE_BIT_BYTE = 8;
        byte[] bytes;

        bytes = this.orderId.getBytes();
        for (byte b : bytes) {
            crc = (crc >>> SIZE_BIT_BYTE) ^ table[(crc ^ b) & 0xFF];
        }

        bytes = this.ripId.getBytes();
        for (byte b : bytes) {
            crc = (crc >>> SIZE_BIT_BYTE) ^ table[(crc ^ b) & 0xFF];
        }

        bytes = ByteBuffer.allocate(SIZE_BYTES_INT).putInt(this.quantity).array();
        for (byte b : bytes) {
            crc = (crc >>> SIZE_BIT_BYTE) ^ table[(crc ^ b) & 0xFF];
        }

        bytes = ByteBuffer.allocate(SIZE_BYTES_INT).putInt(this.overprint).array();
        for (byte b : bytes) {
            crc = (crc >>> SIZE_BIT_BYTE) ^ table[(crc ^ b) & 0xFF];
        }

        bytes = ByteBuffer.allocate(SIZE_BYTES_INT).putInt(this.stepCount).array();
        for (byte b : bytes) {
            crc = (crc >>> SIZE_BIT_BYTE) ^ table[(crc ^ b) & 0xFF];
        }

        bytes = ByteBuffer.allocate(SIZE_BYTES_INT).putInt(this.ovenTemperature).array();
        for (byte b : bytes) {
            crc = (crc >>> SIZE_BIT_BYTE) ^ table[(crc ^ b) & 0xFF];
        }
